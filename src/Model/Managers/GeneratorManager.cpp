...
GeneratorManager::GeneratorManager(sf::Vector2u window_size, EntityManager* entity_manager, Model::GameModel* game_model)
    : window_size(window_size), entity_manager(entity_manager), game_model(game_model), 
      scroll_speed(Utils::GameConfig::getInstance().DEFAULT_SCROLL_SPEED) {
}
...
void GeneratorManager::generateInitialPlatforms() {
    ...
    for (int i = 0; i < initial_platforms; ++i) {
        sf::Vector2f position(
            static_cast<float>(rand() % static_cast<int>(window_size.x - config.PLATFORM_SIZE.x)),
            static_cast<float>(window_size.y / 3 + window_size.y / 2 / initial_platforms * i)
        );
        
        // 初始平台都使用普通平台
        entity_manager->addPlatform(PlatformType::NORMAL, position, config.PLATFORM_SIZE, scroll_speed);
    }
}
...
void GeneratorManager::generatePlatform() {
    ...
    entity_manager->addPlatform(type, position, config.PLATFORM_SIZE, scroll_speed);
}
...
void GeneratorManager::generateEnemy() {
    ...
    switch (side) {
        case 0: // 上边
            position = sf::Vector2f(rand() % window_size.x, -config.ENEMY_SIZE.y);
            break;
        case 1: // 右边
        ...
        case 2: // 下边
            ...
            break;
        case 3: // 左边
            position = sf::Vector2f(-config.ENEMY_SIZE.x, rand() % window_size.y);
            break;
    }
    
    entity_manager->addEnemy(EnemyType::GHOST, position, config.ENEMY_SIZE, game_model);
}
...
void GeneratorManager::generatePickup() {
        ...
        if (!platforms.empty()) {
            ...
            if (platform) {
                ...
                position = sf::Vector2f(
                    platform_pos.x + rand() % static_cast<int>(platform_size_actual.x - config.PICKUP_SIZE.x),
                    platform_pos.y - config.PICKUP_SIZE.y - 10
                );
            }
        } else {
            position = sf::Vector2f(
                rand() % static_cast<int>(window_size.x - config.PICKUP_SIZE.x),
                window_size.y + config.PICKUP_SIZE.y
            );
        }
    } else {
        // 五角星豆子 - 从底部空中生成
    ...
    entity_manager->addPickup(type, position, config.PICKUP_SIZE, game_model, target_platform_id);
}

void GeneratorManager::resetPlatformGenerateInterval() {
    auto& config = Utils::GameConfig::getInstance();
    platform_generate_interval = config.PLATFORM_GENERATE_BASE_INTERVAL + static_cast<float>(rand() % 1 - 0.5) / 2;
}

void GeneratorManager::resetEnemyGenerateInterval() {
    auto& config = Utils::GameConfig::getInstance();
    enemy_generate_interval = config.ENEMY_GENERATE_BASE_INTERVAL + static_cast<float>(rand() % 2); // 3-5秒
}

void GeneratorManager::resetPickupGenerateInterval() {
    auto& config = Utils::GameConfig::getInstance();
    pickup_generate_interval = config.PICKUP_GENERATE_BASE_INTERVAL + static_cast<float>(rand() % 10) / 10.0f; // 0.8-1.8秒
}
