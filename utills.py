...
def string_align_cnter(string, length):
...
class Player:
    ...
    def get_score(self, ba):
        ...
        if self.body.top > ba.rect.top and not ba.score:
            self.hell.score += 1
            self.hell.score_players[self.id] += 1
            ba.score = True
    ...
    def draw(self):
        if self.alive:
            self.screen.fill(self.body_color, self.body)
            for skill in self.skills:
                if skill.product != None:
                    self.screen.fill(skill.product_color, skill.product)
...
class dash(Skill):
    def __init__(self, key=pygame.K_q):
...
class wall(Skill):
    def __init__(self, key=pygame.K_w):
...
class hilaijinnojyutsu(Skill):
    def __init__(self, key=pygame.K_e):
        super(hilaijinnojyutsu, self).__init__(key)
        self.ishilaijin = 0
        self.product_color = 0xFFFF00
        self.cd = 0
